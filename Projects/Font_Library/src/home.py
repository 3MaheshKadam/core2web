...
class C2W_TkinterLibraryApp:
    ...
    def setup_ui(self):
        ...
        img_path = os.path.join(script_directory,'../assets/img/pylogo.png')
    ...
    def c2w_show_font_tab(self):
        if self.font_library_opened:
            ...
            self.c2w_input_label = ttk.Entry(self.c2w_right_frame,width=60,font=("Poppins",12))
            self.c2w_input_label.insert(0,"Enter your text here.....")
            self.c2w_input_label.pack()
            self.c2w_input_label.place(x=60*2 ,y=200,anchor="nw")
            ...
            self.c2w_button_label4 = ttk.Button(self.c2w_right_frame,text="Serif" ,style="Rounded.TButton", command=self.c2w_sanserifClicked)
    ...
    def c2w_poppinClicked(self):
        self.c2w_show_label.config(text=self.c2w_input_label.get(),font=("Poppins",30,"bold"))
    
    def c2w_modernClicked(self):
        self.c2w_show_label.config(text=self.c2w_input_label.get(),font=("Modern",30,"bold"))

    def c2w_courierClicked(self):
        self.c2w_show_label.config(text=self.c2w_input_label.get(),font=("Courier",30,"bold"))

    def c2w_sanserifClicked(self):
        self.c2w_show_label.config(text=self.c2w_input_label.get(),font=("MS Sans Serif",30,"bold"))

    def c2w_calibriClicked(self):
        self.c2w_show_label.config(text=self.c2w_input_label.get(),font=("Calibri",30,"bold"))
